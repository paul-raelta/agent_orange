/* Agent Orange — mock data layer.
   This is intentionally a thin, self-contained "fixture" that mimics what the
   agentic backend would return. The frontend only ever talks to window.AO_DATA,
   so the real backend can be swapped in behind the same shape with zero UI change. */
(function () {
  // ---- Provenance snippets (where a number was found / corroborated) ----
  const NVDA_NIPS_SNIPPET = {
  ...
  const SNDK_PR_SNIPPET = {
    source: "Press release — fiscal Q4 results",
    url: "investors.sandisk.com/news/.../fy-q4",
    page: 1,
    quote:
      "Revenue of $1.95 billion. GAAP net income of $0.82 per diluted share.",
  };
  const SNDK_TABLE_SNIPPET = {
  ...
  const companies = [
    {
      ...
      status: "validated", // validated | review | watching | error
      sourceMode: "auto", // auto | guided | advanced
      sources: [
  ...
  window.AO_DATA = { companies, reviewQueue, activity, usage, providers, routing };
})();

"""Help-assistant grounding corpus — the SINGLE SOURCE OF TRUTH the LLM sees.

The entire `AO_KB_TEXT` block is injected into the system prompt on every
question, so the assistant can answer ONLY from these verified facts and never
invents features.

This file is the Python port of `design/help/agent/knowledge.js`. The structure
is identical and the serialization in `AO_KB_TEXT` matches the JS verbatim so
the prompt is byte-equivalent. Keep this file in lockstep with knowledge.js —
if you edit one, edit the other. The static `/help` guide (web/public/help/)
covers complementary annotation content.
"""
...
AO_KB: dict = {
    "product": {
        "name": "Agent Orange",
        "oneLiner": (
            "Autonomous earnings intelligence — an AI agent watches each company you track, "
            "reads its earnings filings the moment they drop, extracts the figures that move trades, "
            "and validates every number against a second source before showing it to you."
        ),
        "who": "Built for investors who trade around earnings and need trustworthy numbers fast, with receipts.",
        "loop": [
            "LOCATE — each company gets its own agent that knows where its results live: the company "
            "investor-relations (IR) site, with SEC EDGAR as the structured backbone.",
            "MONITOR — earnings dates are unpredictable, so the agent polls on a cadence that intensifies "
            "inside the predicted filing window.",
            "EXTRACT — it pulls the figures that drive trades: revenue, net income, EPS (basic & diluted), "
            "gross margin, and guidance.",
            "VALIDATE — it cross-references each figure in two or more places; agreement earns a confidence "
            "level, a conflict is flagged.",
            "SURFACE — every number is shown with full provenance (source, page, exact quote); anything it "
            "can’t confidently validate is routed to you in the Review queue.",
        ],
        "principle": (
            "Trust through provenance: the agent never just asserts a number — every figure links back to "
            "the exact place it was read, and confidence is earned by corroboration. When sources disagree, "
            "a human decides; the agent never guesses."
        ),
    },
    ...
    "screens": [
        {
            "name": "Watchlist", "route": "the home screen",
            "purpose": "At-a-glance status of every company you track.",
            "parts": [
                "Status summary line (how many watching / need review / validated)",
                "RUN ALL AGENTS button (top right)",
                "A portfolio P&L strip if you hold positions",
                "A company card per company: status bar + chip, live price + EPS sparkline, latest period, a 3-metric grid with YoY and confidence, and a status-specific footer",
            ],
            "actions": [
                "Click a card to open its deep-dive",
                "Press RUN ALL AGENTS to check everything now",
                "Read the left color bar / status chip to triage",
            ],
        },
        {
            "name": "Company deep-dive", "route": "opens when you click a card",
            "purpose": "Everything about one company: history, sources, and how each figure was validated.",
            "parts": [
                "Header: ticker, sector, cadence, price, status",
                "SOURCES row: IR site (primary) + SEC EDGAR CIK, and the source mode",
                "AI narrative card (\"what’s worth knowing\")",
                "Editable position (shares + cost basis → live P&L)",
                "Tabs: RESULTS, VALIDATION, NEWS, INSIDER, AGENT RUNS",
                "RESULTS: last-5-periods table, latest column highlighted",
                "VALIDATION: pass/fail card + per-metric confidence rows",
            ],
            "actions": [
                "Switch tabs to see results vs validation vs news",
                "Click a confidence badge or metric row to open the Provenance drawer",
                "Edit shares/cost basis to track your position",
            ],
        },
        {
            "name": "Provenance drawer", "route": "slides in from the right when you click a confidence badge or metric row",
            "purpose": "Show exactly where a number came from.",
            "parts": [
                "The figure + its confidence + YoY at the top",
                "One block per source: document title, page number, URL, and the exact quoted snippet",
                "Multiple agreeing sources = higher confidence",
            ],
            "actions": [
                "Read each snippet to see the number in context",
                "Follow a URL to the original document",
                "Press Esc or ✕ to close",
            ],
        },
        {
            "name": "Timeline", "route": "Timeline in the sidebar",
            "purpose": "See when results are expected for each company.",
            "parts": [
                "A month track across the top with a \"NOW\" marker",
                "One lane per company",
                "Green dots = already reported & recorded",
                "Orange bars = predicted filing window",
                "Amber pulsing bars = an agent watching right now",
            ],
            "actions": [
                "Look for amber bars — those filings are imminent",
                "Use predicted windows to anticipate results",
                "Click a lane to jump to that company",
            ],
        },
        {
            "name": "Review queue", "route": "Review in the sidebar (shows a badge count)",
            "purpose": "Decide on figures the agent could not confidently validate.",
            "parts": [
                "One card per finding: company, period, the disputed metric, and the reason",
                "Candidate values side by side, each with its source and a note",
                "The exact provenance snippet behind the conflict",
                "Decision buttons: USE <value> or REJECT",
            ],
            "actions": [
                "Read the reason and compare candidate values",
                "Check the snippet to understand the discrepancy",
                "Click USE to record the right value, or REJECT to discard",
            ],
        },
        {
            "name": "Companies", "route": "Companies in the sidebar",
            "purpose": "Manage tracked companies and add new ones.",
            "parts": [
                "A list of configured companies (sources, cadence, mode, status)",
                "An ADD COMPANIES button opening the S&P 500 browse flow",
            ],
            "actions": [
                "Click ADD COMPANIES to browse and add",
                "Click a row to open that company",
            ],
        },
        {
            "name": "Add companies (browse flow)", "route": "from Companies → ADD COMPANIES",
            "purpose": "Add many companies at once from the S&P 500.",
            "parts": [
                "Search (ticker or name) + sort (market cap / A–Z / soonest earnings)",
                "A Grid ⇄ Table density toggle",
                "Sector filters and sector groups, each with \"Select all\"",
                "Company cards with price, market cap, next-earnings date",
                "A selection tray with the ADD button",
            ],
            "actions": [
                "Search or filter, then click cards to select",
                "Use Select all for a whole sector",
                "Press ADD — agents discover each company’s sources, then you confirm to start watching",
            ],
        },
        {
            "name": "Activity log", "route": "Activity in the sidebar",
            "purpose": "A transparent, auditable feed of everything the agents did.",
            "parts": [
                "A filter bar (all, or per company)",
                "Rows: timestamp, agent, plain-language message, and tokens · cost",
            ],
            "actions": [
                "Filter to one company to follow its agent",
                "Read top-down to reconstruct what happened",
                "Use the cost column to see where compute went",
            ],
        },
        {
            "name": "Settings", "route": "Settings in the sidebar",
            "purpose": "Control cost, models, schedules, notifications, and optional features.",
            "parts": [
                "USAGE: month-to-date spend vs budget, tokens, runs, per-model breakdown",
                "PROVIDERS: Anthropic Claude (active); OpenAI & Google (planned)",
                "MODEL ROUTING: pick Haiku / Sonnet / Opus per task (discovery, monitoring, extraction, validation)",
                "NOTIFICATIONS: email + SMS, with per-trigger toggles",
                "RUN-ALL FEEDBACK: how the app confirms a kicked run (toast / held button / both)",
                "FIRST-TIME EXPERIENCE: reset to first-time state",
                "LABS: feature flags for Consensus, Conflict workspace, and Guidance",
                "SCHEDULE & VALIDATION DEFAULTS: poll cadence, run mode, default validation rule",
            ],
            "actions": [
                "Set a monthly budget and watch the usage bar",
                "Route cheap models to discovery/monitoring, strong models to extraction/validation",
                "Set up email/SMS alerts",
                "Toggle Labs features on or off",
            ],
        },
        {
            "name": "Document Examiner (Run all agents)", "route": "opens full-screen when you press RUN ALL AGENTS",
            "purpose": "Watch the agents actually read the filings.",
            "parts": [
                "The filing fills the screen as real \"paper\"",
                "A lens zooms to each section; figures are circled on the page",
                "An Extracted Data panel fills with captured figures + confidence",
                "A telemetry rail: pages read, tables parsed, figures captured, sources cross-referenced, model spend",
                "A per-company progress rail",
            ],
            "actions": [
                "Press RUN ALL AGENTS to start it",
                "Press ▾ to minimize and keep using the app while it runs in the background",
                "Press ⤢ to expand back; it ends with a summary and syncs the Watchlist",
            ],
        },
        {
            "name": "Tweaks (personalization)", "route": "the Tweaks panel",
            "purpose": "Adjust the look and feel; changes apply live and are remembered.",
            "parts": [
                "Accent color (4 curated)",
                "Surface: Carbon / Slate / Black",
                "Mono typeface: Plex / JetBrains / Space",
                "Density: Cozy / Compact",
                "Card sparklines toggle",
            ],
...
def _serialize(kb: dict) -> str:
    """Mirror of the JS `AO_KB_TEXT` block. Produces the same fenced corpus
    that the prototype injected, line for line."""
    L: list[str] = []
    L.append("# AGENT ORANGE — PRODUCT")
    L.append(kb["product"]["oneLiner"])
    L.append("Audience: " + kb["product"]["who"])
    L.append("The per-company agent loop: " + " ".join(kb["product"]["loop"]))
    L.append("Core principle: " + kb["product"]["principle"])

    L.append("\n# GLOSSARY")
    for t, d in kb["glossary"]:
        L.append(f"- {t}: {d}")

    L.append("\n# SCREENS & FEATURES")
    for s in kb["screens"]:
        L.append(f"## {s['name']} ({s['route']}) — {s['purpose']}")
        L.append("  Parts: " + "; ".join(s["parts"]))
        L.append("  You can: " + "; ".join(s["actions"]))
    ...
    L.append("\n# TROUBLESHOOTING")
    for q, a in kb["troubleshooting"]:
        L.append(f"- {q}: {a}")

    return "\n".join(L)


AO_KB_TEXT: str = _serialize(AO_KB)
